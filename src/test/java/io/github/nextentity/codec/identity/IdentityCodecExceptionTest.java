...
/**
 * IdentityCodecException 测试类
 * 测试身份编码异常的各种构造函数和功能
 */
@SuppressWarnings("SpellCheckingInspection")
public class IdentityCodecExceptionTest {

    private static final Logger logger = LoggerFactory.getLogger(IdentityCodecExceptionTest.class);
    ...
    void testBasicConstructor() {
        ...
        IdentityCodecException exception = new IdentityCodecException(message);
    ...
    void testConstructorWithErrorCodeAndCause() {
        ...
        IdentityCodecException exception = new IdentityCodecException(errorCode, detail, cause);
     ...
     */
    @Test
    void testAllEncodingErrorCodeEnums() {
        // 测试编码相关的错误码
    ...
    void testExceptionSerialization() {
        ErrorCode errorCode = ErrorCode.UNSUPPORTED_VERSION;
        String detail = "序列化测试";
        IdentityCodecException originalException = new IdentityCodecException(errorCode, detail);
    ...
    void testExceptionInheritance() {
        ...
        // 验证继承关系
        assertInstanceOf(RuntimeException.class, exception, "IdentityCodecException应该继承RuntimeException");
        assertInstanceOf(Exception.class, exception, "IdentityCodecException应该继承Exception");
    ...
    void testExceptionStackTrace() {
        ...
        try {
            // 故意抛出异常以生成堆栈跟踪
            throw new IdentityCodecException(errorCode, detail);
        } catch (IdentityCodecException exception) {
    ...
    void testExceptionEqualsAndHashCode() {
        ...
        IdentityCodecException exception1 = new IdentityCodecException(message1);
        IdentityCodecException exception2 = new IdentityCodecException(message1);
        IdentityCodecException exception3 = new IdentityCodecException(message2);
    ...
    void testRealWorldUsage() {
        ...
        try {
            ...
            assertTrue(e.getMessage().contains("IDC-001"));
            assertTrue(e.getMessage().contains("版本 2 不被支持"));
            assertEquals(ErrorCode.UNSUPPORTED_VERSION, e.getErrorCode());
        }
        ...
        try {
            ...
            assertTrue(e.getMessage().contains("IDC-002"));
            assertTrue(e.getMessage().contains("预留位包含非零值"));
            assertEquals(ErrorCode.RESERVED_BITS_NOT_ZERO, e.getErrorCode());
        }
    ...
    void testChainedExceptionHandling() {
        ...
        IdentityCodecException chainedException = new IdentityCodecException(errorCode, detail, rootCause);
    ...
    /**
     * 测试 EncryptorException 继承关系
     */
    @Test
    void testEncryptorExceptionInheritance() {
        String message = "加密器测试";
        EncryptorException exception = new EncryptorException(message);

        assertInstanceOf(IdentityCodecException.class, exception, "EncryptorException应该继承IdentityCodecException");
        assertInstanceOf(RuntimeException.class, exception, "EncryptorException应该继承RuntimeException");

        logger.info("EncryptorException继承关系测试通过");
    }

    /**
     * 测试 InvalidIdentityNumberException 继承关系
     */
    @Test
    void testInvalidIdentityNumberExceptionInheritance() {
        String message = "身份证号码测试";
        ...
        logger.info("InvalidIdentityNumberException继承关系测试通过");
    }
}
